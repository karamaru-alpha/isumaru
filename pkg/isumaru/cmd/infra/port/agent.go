...
func (p *agentPort) PProf(_ context.Context, agentURL string, duration time.Duration) (io.ReadCloser, error) {
	// ブラウザのリロードでcancelされないように別のctxを使用する
	ctx := context.Background()
	req, err := http.NewRequestWithContext(
		ctx,
		http.MethodGet,
		fmt.Sprintf("%s/debug/fgprof?seconds=%d", agentURL, int(duration.Seconds())),
		nil,
	)
	if err != nil {
		return nil, err
	...
	if resp.StatusCode != http.StatusOK {
		return nil, errors.New(fmt.Sprintf("fail to collect pprof. err=%+v", resp.Body))
	}
	if resp.ContentLength == 0 {
		return nil, errors.New("log is empty")
	}
